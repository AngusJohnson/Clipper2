...
namespace Clipper2Lib
{
  ...
  public class SimpleClipperSvgWriter
  {
    ...
    public string SaveToString(int maxWidth = 0, int maxHeight = 0, int margin = -1)
    {
      ...
      double scale = 1.0;
      if (maxWidth > 0 && maxHeight > 0)
        scale = Math.Min((maxWidth - margin * 2) / (double) bounds.Width, (maxHeight - margin * 2) / (double) bounds.Height);
      ...
      if (maxWidth <= 0 || maxHeight <= 0)
        sb.AppendFormat(svg_header1, (bounds.right - bounds.left) + margin * 2, (bounds.bottom - bounds.top) + margin * 2);
      else
        sb.AppendFormat(svg_header1, maxWidth, maxHeight);
      ...
      foreach (PolyInfo pi in PolyInfoList)
      {
        ...
        foreach (PathD path in pi.paths)
        {
          if (path.Count < 2) continue;
          if (!pi.IsOpen && path.Count < 3) continue;
          sb.AppendFormat(NumberFormatInfo.InvariantInfo, " M {0:f2} {1:f2}", path[0].x * scale + offsetX, path[0].y * scale + offsetY);
          for (int j = 1; j < path.Count; j++)
          {
            sb.AppendFormat(NumberFormatInfo.InvariantInfo, " L {0:f2} {1:f2}", path[j].x * scale + offsetX, path[j].y * scale + offsetY);
          }
          if (!pi.IsOpen) sb.Append(" z");
        }

        if (!pi.IsOpen)
          sb.AppendFormat(NumberFormatInfo.InvariantInfo, svg_path_format, ColorToHtml(pi.BrushClr), GetAlpha(pi.BrushClr), FillRule == FillRule.EvenOdd ? "evenodd" : "nonzero", ColorToHtml(pi.PenClr), GetAlpha(pi.PenClr), pi.PenWidth);
        else
          sb.AppendFormat(NumberFormatInfo.InvariantInfo, svg_path_format2, ColorToHtml(pi.PenClr), GetAlpha(pi.PenClr), pi.PenWidth);

        if (pi.ShowCoords)
        {
          sb.AppendFormat(NumberFormatInfo.InvariantInfo, "<g font-family=\"{0}\" font-size=\"{1}\" fill=\"{2}\">\n", coordStyle.FontName, coordStyle.FontSize, ColorToHtml(coordStyle.FontColor));
          foreach (PathD path in pi.paths)
          {
            foreach (PointD pt in path)
            {
#if USINGZ
              sb.AppendFormat(NumberFormatInfo.InvariantInfo, "<text x=\"{0}\" y=\"{1}\">{2},{3},{4}</text>\n", (int) (pt.x * scale + offsetX), (int) (pt.y * scale + offsetY), pt.x, pt.y, pt.z);
#else
              sb.AppendFormat(NumberFormatInfo.InvariantInfo, "<text x=\"{0:f2}\" y=\"{1:f2}\">{2},{3}</text>\n", (pt.x * scale) + offsetX, (pt.y * scale) + offsetY, pt.x, pt.y);
#endif
            }
          }
          sb.Append("</g>\n\n");
      ...
      foreach (TextInfo captionInfo in textInfos)
      {
        sb.AppendFormat(NumberFormatInfo.InvariantInfo, "<g font-family=\"Verdana\" font-style=\"normal\" font-weight=\"normal\" font-size=\"{0}\" fill=\"{1}\">\n", captionInfo.fontSize, ColorToHtml(captionInfo.fontColor));
        sb.AppendFormat(NumberFormatInfo.InvariantInfo, "<text x=\"{0}\" y=\"{1}\">{2}</text>\n</g>\n", captionInfo.posX + margin, captionInfo.posY + margin, captionInfo.text);
      }
