...
**Simple polygons** are ones that don't self-intersect and have no holes. **Complex polygons** are polygons that aren't *simple*.**Polygon holes** are regions inside polygons that aren't filled. *Holes* are usually defined by *inner* polygons that are separate from and surrounded by *outer* polygons. While the filling regions of *simple polygons* are unambiguous, filling regions of *complex polygons* are not. When there's more than one enclosed region, additional information is required to fully define which of these regions are filled, and which are not (**filling rules**). There are two commonly used filling rules - *EvenOdd* and *NonZero* - though there are others too, also supported by the library.<br>
**Segments** are the lines between path vertices. In closed paths (ie polygons) these are commonly referred to as **edges**.<br>
Edges are considered **touching** when they are collinear and at least partially overlap one another.<br>
Polygons are *touching* when one polygon has an edge *touching* an edge in the other polygon.<br><br>
