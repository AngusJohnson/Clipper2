...
namespace Clipper2Lib {
  ...
  inline bool UpdateHorzSegment(HorzSegment& hs, int64_t  curr_y)
  {
    ...
    OutPt* op = hs.outrec->pts, *op2 = op->next;
    if (hs.outrec->skip || op == op2) return false;
    ...
    bool is_bottom = op2->next == op;    
    
    if (op == op->next) return false;
    bool ltor;
    if (is_bottom)
    {
    ...
    hs.leftToRight = ltor;
    hs.outrec->skip = true; // block subsequent 
    return true;
  }
  ...
  void ClipperBase::MergeHorzSegments(int64_t curr_y)
  {
    ...
    {
      if (UpdateHorzSegment(hs, curr_y))
        horz_seg_list_[j++] = hs;
    }
    ...
    for (; hs != hs_last; ++hs)
    {
      ...
      bool isMax1 = !or1->front_edge;

      for (hs2 = hs +1; hs2 != hs_end; ++hs2)
      {
        OutRec* or2 = hs2->outrec;
        if (hs->leftToRight == hs2->leftToRight) continue;
        ...
        bool isMax2 = !or2->front_edge;
        OutPt *left1, *left2, *right1, *right2;
        if (hs->leftToRight)
        {
          right1 = hs->rightOp;
          right2 = hs2->rightOp;
          while (right1->prev->pt.x >= right2->pt.x)
            right1 = right1->prev;
          while (right2->next->pt.x >= right1->pt.x)
            right2 = right2->next;
          left1 = right1->prev;
          left2 = right2->next;
        }
        else
        {
          right1 = hs->rightOp;
          right2 = hs2->rightOp;
          while (right1->next->pt.x >= right2->pt.x)
            right1 = right1->next;
          while (right2->prev->pt.x >= right1->pt.x)
            right2 = right2->prev;
          left1 = right1->next;
          left2 = right2->prev;
        }
  ...
  {
    op.outrec->skip = false;
    horz_seg_list_.push_back(HorzSegment(op.outrec));
  }
