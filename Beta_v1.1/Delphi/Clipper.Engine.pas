...
type
  ...
  TOutRec = record
    ...
    path     : TPath64;
    isOpen   : Boolean;
    horzSkip : Boolean; // see MergeHorzSegs()
  end;
...
begin
  ...
  if IsJoined(e1) then Split(e1, pt); // todo - ?? remove these
  ...
  begin
    ...
    begin
      //outRec.owner := GetRealOutRec(outRec.owner); //do after CleanCollinear later
...
procedure TClipperBase.Split(e: PActive;
  const currPt: TPoint64);
begin
...
begin
  if not IsHotEdge(e) or not Assigned(e.nextInAEL) or
...
procedure CheckOutPts(outrec: POutrec);
var
  op: POutPt;
begin
  op := outrec.pts.next;
...
procedure UpdateOutrecs(op: POutPt; outrec: POutrec);
begin
  while op.outrec <> outrec do
  begin
    op.outrec := outrec;
    op := op.next;
  end;
...
function CheckHorzPrev(thisOp, refOp: POutPt): Boolean;
  {$IFDEF INLINING} inline; {$ENDIF}
var
  op: POutPt;
begin
  ...
  op := thisOp;
  repeat
     if op = refOp then Exit;
     op := op.prev;
     if op.pt.Y <> thisOp.pt.Y then break;
  until op = thisOp;
  Result := true;
end;
//------------------------------------------------------------------------------

function CheckHorzNext(thisOp, refOp: POutPt): Boolean;
  {$IFDEF INLINING} inline; {$ENDIF}
var
  op: POutPt;
begin
...
var
  op, op2: POutPt;
  outrec: POutrec;
  ltor, isBottom: Boolean;
begin
  hs.outrec := GetRealOutRec(hs.outrec);
  outrec := hs.outrec;
  result := false;

  if outrec.horzSkip or
    (outrec.pts = outrec.pts.next) then
      Exit;
...
var
  ...
  or1, or2: POutrec;
  isMax1, isMax2: Boolean;
  e: PActive;
begin

  j := 0;
  // since each OutRec can only have one active horizontal segement
  // UpdateHorzSegment() will return false if that horizontal segment
  ...
  begin
    ...
    isMax1 := not Assigned(or1.frontE);
    ...
    begin
      ...
      if (hs2.leftToRight = hs1.leftToRight) then
        Continue;
      ...
      isMax2 := not Assigned(or2.frontE);

      if hs1.leftToRight then
      begin
        right1 := hs1.rightOp;
        right2 := hs2.rightOp;
        while right1.prev.pt.X >= right2.pt.X do
          right1 := right1.prev;
        while right2.next.pt.X >= right1.pt.X do
          right2 := right2.next;
      ...
      begin
        right1 := hs1.rightOp;
        right2 := hs2.rightOp;
        while right1.next.pt.X >= right2.pt.X do
          right1 := right1.next;
        while right2.prev.pt.X >= right1.pt.X do
          right2 := right2.prev;
      ...
      if not isMax1 and not isMax2 then
      begin
