...
int main(int argc, char* argv[])
{
  ...
  SvgSaveToFile(svg, "solution_off.svg", fr, 800, 600);
  ...
  while (p.size())
  {
    ...
    p = InflatePaths(p, -250, JoinType::Round, EndType::Polygon);
    //RamerDouglasPeucker - not essential but
    //speeds up the loop and also tidies up the result
    p = RamerDouglasPeucker(p, 1);
    pp.insert(pp.end(), p.begin(), p.end());
  }

  svg.Clear();
  SvgAddSolution(svg, ScalePathsD(pp, 0.01), false);      //scale back down
  SvgSaveToFile(svg, "solution_off2.svg", fr, 800, 600);
